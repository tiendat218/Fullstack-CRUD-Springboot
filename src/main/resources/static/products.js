...
var productService = {
    ...
    update(id, product, fn){
        axios
            .get('/api/v1/products' + id, product)//|IDBRequest<..> | Promise<void>
            .then(response => fn(response))//Promise<T>
            .catch(error => console.log(error))
    },
    deleteProduct(id,fn){
        axios
...
var Product = Vue.extend({
    template : '#product',
    data: function () {
        return {product: findProduct(this.$router.params.product_id)};
    }
});
var ProductEdit = Vue.extend({
    tempalte:'#product-edit',
    data:function () {
        return {product:findProduct(this.$router.params.product_id)};
    },
    methods:{
